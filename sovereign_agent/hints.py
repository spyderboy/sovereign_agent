...
ERROR_HINTS: list[tuple[str, str]] = [
    ...
    (r"positional argument.*AstroGame|AstroGame.*positional argument|"
     ...
     "  CORRECT:\n"
     "    // Inside a Provider or ConsumerWidget:\n"
     "    final game = AstroGame(ref);\n"
     "    // Or via provider:\n"
     "    final gameProvider = Provider<AstroGame>((ref) => AstroGame(ref));\n"
     "  WRONG: AstroGame()  ← missing required Ref argument\n"
     "  In tests, use a ProviderContainer and pass container.read as a Ref-compatible object,\n"
     "  OR mock AstroGame rather than constructing it directly.\n"
     "  The class signature is:  class AstroGame extends FlameGame with ScaleDetector {\n"
     "    AstroGame(this._ref);  // _ref is a Riverpod Ref\n"
     "  }"),
]
